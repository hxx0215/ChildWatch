...
-(void)setWithMixNuber:(NSInteger)mixNuber maxNuber:(NSInteger)maxNuber showType:(rulerViewShowType)showType rulerMultiple:(NSInteger)rulerMultiple
{
    self.clipsToBounds=YES;
    _round=NO;
    _mixNuber=mixNuber;
...
-(void)setDefaultVaule:(CGFloat)defaultVaule{
    ...
    CGFloat gapValue=(_showType==rulerViewshowHorizontalType)?(defaultVaule-_mixNuber+((CGFloat)_rulerMultiple/2))*formlength:(_maxNuber-defaultVaule+((CGFloat)_rulerMultiple/2))*formlength;
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if ([self.delegate respondsToSelector:@selector(getRulerValue:withScrollRulerView:)]) {
        ...
        if (_round) {
            value=(_showType==rulerViewshowHorizontalType)?_mixNuber+round(scrollValue):_maxNuber-round(scrollValue)-1;
        }else{
            value=(_showType==rulerViewshowHorizontalType)?_mixNuber+scrollValue:_maxNuber-scrollValue-1;
        }
